...
- キャンバスが外れる導線は `App` にある（ボードの切り替え、新しいボードの作成先
  選択、ログアウト、**ブラウザの戻る / 進む**）。**通す前に `confirmDiscard` を
  通す。** 導線を足したらここにも足す。「作成先を変更」だけは `dirty` でボタンを
  止める形で揃えてある。
  - **戻る / 進むだけは形が違う**（ADR 0059）。`popstate` が届いた時点で
    ブラウザはもう動いていて、止める手立てが無い。捨てないと言われたら
    **見えている場所を積み直して URL を画面に合わせる。** `history.back()` で
    戻す形は採らない。何手ぶん動かされたのかを知る手立てが無く、戻す量を
    決められない。
...
- **state から URL を導く `useEffect` を置かない。** effect では「積むのか
  ...
  競合する。**導線ごとに `showLocation` を呼ぶ。** 未保存の確認を導線ごとに
  通しているのと同じ形で、**導線を足したらここにも足す。**
- **積むのはボードの切り替えだけ。** 選び直し・作成中・削除・ログアウト・
  起動時の復元は置き換える。引き返せない場所（消えたボード、ログアウト前）を
  積むと、押した先で画面と URL が食い違う。
- **URL から選び直しに入れる条件は、`BoardPage` が「作成先を変更」を出す条件と
  同じ。** 緩めると、画面から押せないはずの操作に URL からだけ入れる
  （中核思想 3）。**ずれたことは `web/e2e/url.spec.ts` が落とす。**
  - **落とした指定の理由は別に書かない。** 落ちた先のボードには
    `BoardPage` が理由を出している（「作成先を変えられるのはオーナーだけです」
    「作成先は確定」）ので、ADR 0030 の「押す前に理由を見せる」は満たされる。
    ここで足すと、同じ理由が画面に 2 つ並ぶ。
- **開けなかったボードを URL に残さない。** 残すと、読み込み直すたびに同じ
  失敗を繰り返す。非メンバーにも消えたボードにも同じ `not_found` が返るので
  （ADR 0016 / 0017）、**URL から開いたときも見せ方を変えない。**
- **ログイン後の戻り先を持つのはサーバー**（state と一緒）。フロントが渡すのは
  ログインを始める 1 回だけで、認可の往復の URL には載らない。自オリジンの
  相対パス以外はサーバーが 400 で弾く。
