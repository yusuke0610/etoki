...
- **保存は「何の上に描いたか」を持ち回る（ADR 0020）。** `BoardDetail.updatedAt`
  ...
  相手ではなくこちらの作業になる。
- **シーンの大きさの上限はサーバーだけが持つ。** ヘッダーに出しているのは
  いまのバイト数（`web/src/excalidraw/size.ts`）という状態で、判定ではない。
  **上限との比（「8 MiB 中 …」）を出さない。** 比を出すには上限をフロントが
  知る必要があり、それは判定を 2 箇所に持つのと同じことになる（ADR 0018）。
