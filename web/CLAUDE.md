...
- **注釈のメタデータの形も Go 側と同じにする。** `web/src/excalidraw/annotation.ts` の
  ...
  図の種別（`kind`）が載る（ADR 0047）。**フロントだけが読める形で持たない。**
...
- **「未保存」判定（`web/src/excalidraw/dirty.ts`）は `content_hash` とは別物。**
  ...
  変更を取りこぼす。**署名に入れる基準は「保存が書くかどうか」。** 要素のほかに
  背景色（`viewBackgroundColor`）が入っているのはそのため（ADR 0045）。スクロール
  位置や選択は保存が書かないので入れない。入れると開いただけで未保存になる。
...
- **frame を作らない。** etoki が frame を自前生成しない線を、ひな形のためにも
  曲げない。**どこを issue 化の対象にするかは人がフレームツールで囲んで決める。**
  その帰結として、ひな形から作ったボードは開いた直後に注釈を 1 つも持たない。
- **種別（`kind`）はひな形には載らない。** 載る先は人が引いた frame なので、
  選ぶのは注釈パネルの「種別」（粒度の隣）。**ひな形の選択と自動で結ばない**
  （中核思想 3）。結ぶと、ひな形をなぞらずに別の図を描く自由をその場で奪う。
