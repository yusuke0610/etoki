...
- **部品の共通の見た目（`button` / `input` / `select` など）は `.excalidraw` の
  中に掛けない。** ライブラリの UI も素の要素なので、掛けるとキャンバスの
  ツールバーまで etoki の形になる。**`:where()` で包んで詳細度を 0 に保つ。**
  用途ごとの規則（`.board-list button` など）が常に勝つようにするため。
  **共通の見た目は文字の途中で折り返さない（`nowrap`）。** 名前や説明をそのまま
  並べる一覧のボタン（`.plain-list button`）は、用途ごとの規則で折り返させる。
  外すと長い名前がボタンの外にはみ出す（#161）。
- **主な操作（`button.primary`）は 1 つの領域に 1 つ。** 並べると次に押すものが
  読めなくなる。解釈するは注釈ごとに並ぶので普通のボタンのまま。
- **切り替えの口は Excalidraw のメニューだけ。** `theme` を渡すとライブラリは
  既定でその項目を隠すので、`UI_OPTIONS` の `toggleTheme: true` を外さない。

## ボード画面の配置（#62）

- **ヘッダーは 2 段。** 上段（`.board-header-main`）は作成先・名前・ロールと、
  大きさ・未保存・保存。下段（`.board-actions`）はボードに対する操作を群に分けて
  並べる。**保存は上段から動かさない。** 下段の操作と同じ列に戻すと、主な操作が
  補助の操作に埋もれる。
- **下段の操作はメニューに畳まない。** 畳むと、押せない理由を畳んだ中でも読ませる
  作りが要り（ADR 0039）、1 手も遠くなる。操作を足すなら意味の近い群に入れる。
- **注釈は状態でまとめて並べる**（`web/src/board/annotationGroups.ts`）。**まとめる
  のは見せ方だけで、状態の判定には触らない。** 組の中の並びは変えず、空の組は
  出さない。
...
- **文字色を薄くするときは AA の 4.5:1 を切らせない。** 押せない理由を
  ...
  切らせない。** axe は両方で掛けてある。
- **入力欄の枠は、載る地に対して 3:1 を切らせない**（WCAG 1.4.11、#161）。
  空の入力欄は枠だけが境界になる。**axe はこの比を測らない**ので、枠の色は
  `--color-input-border` を使い、区切り線用の `--color-border` を流用しない。
