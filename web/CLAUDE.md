...
- **シーンから消えた注釈は別の節に出す**（ADR 0046）。3 状態も名前も無く、解釈も
  ...
  本当に何か残っているときに気づけない。
- **届いたか分からない書き込みは「在るもの」と混ぜない**（ADR 0056）。
  `unconfirmedItems` は `items` とは別のリストで届く。**件数にも数えない**
  （`countByAction` は `confirmed` だけを見る）。混ぜると「N 件は作成済み」が
  嘘になる。
  - **「失敗しました」と書かない。** GitHub が受理したあとで応答だけを失った
    場合も、受理せずに返した場合も区別できない。分からないことを分からないと
    出し、確かめるのは開発者（中核思想 3）。出口は GitHub を見ること 1 つなので、
    リンクを添える。
  - **畳まない。** 「GitHub にある N 件」は開いて確かめるものだが、こちらは
    開発者が手を打つまで消えない。`details` に入れると、開かない限り気づけない。
  - **送り先の分からない項目は選び直させない**（`canResend`）。受理されていた
    場合、もう一度送ると消せない draft issue が重複する。**更新は止めない。**
    相手の ID が分かっているので、書き直しても重複しない。
  - **押せない理由は本文に出す**（ADR 0039）。`id` は `localId` だけでは足りない。
    解釈の中でしか一意でないので、注釈ごとに分ける。
