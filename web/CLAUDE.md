...
- **「未保存」判定（`web/src/excalidraw/dirty.ts`）は `content_hash` と揃えない**
  ...
  開いただけで未保存になる。
  **入口ごとに渡す。** `BoardPage` の `handleChange` / `updateElements` と
  `useSceneSave` の `save` のどれかで渡し忘れると、同じシーンの署名が入口に
  よって食い違う。**署名は区切り
...
- **ヘッダーに出しているのは判定ではなく、いまのバイト数**
  ...
  - **保存できるかどうかの判定結果（真偽値）は `board.sceneOverLimit` として
    ...
    持たない。** `useSceneSave` は `board.sceneOverLimit` を初期値にし、
...
- `dirty` のあいだと、作成の実行中だけ `beforeunload` を登録する
  （`useDirtyScene` の `useConfirmLeave`。**2 つを渡すのは `BoardPage`**）。
...
- **取り込みの確認だけは `useBoardTransfer` が出す**（ADR 0045）。キャンバスは外れない
  が中身は置き換わるので、失うものは同じ。**ファイルを読んでから訊く。**
- **確認の裏で保存しない。** 捨てるつもりの試し描きまで残る。
- 未保存かどうかを決めるのは `BoardPage` の `useDirtyScene`。`App` は
  `onDirtyChange` で受け取るだけで、自前で判定し直さない。作り直すと
  「未保存」の定義が 2 箇所になる。
...
- **一時的に押せないだけでも理由は本文で出す。** 保存中は作成させず、作成中は
  ...
  **何と何が排他かと、そのときの文言は `web/src/board/exclusion.ts` の表が
  正本**（ADR 0060）。ヘッダーもパネルもそこから引く。**条件を組み直さない。**
...
`BoardPage` と注釈パネルは関心ごとのフックとファイルに割ってある。**割り方に
ついての約束が 3 つある。**

- **フックが返す束は `useMemo` で同一性を保つ。** 毎レンダーで作り直すと、
  束の中のコールバックを依存に置いた `useCallback` まで作り直される。
  `handleChange` がそれで毎回差し替わり、**Excalidraw の `onChange` が
  差し替わり続けて `Maximum update depth exceeded` で画面ごと落ちた。**
  vitest も `tsc` も eslint も通ったままなので、**気づけるのは E2E だけ。**
- **依存配列には束ではなく、束の中の安定した値を置く。** `sceneSize` を丸ごと
  依存にすると、バイト数が変わるたびに同じことが起きる。呼ぶ側で
  `const { schedule } = useSceneSize(api)` のように取り出す。
- **切り出すと、それまで効いていなかった lint が効き始める。**
  `react-hooks` の解析は大きすぎる関数を諦めるので、1,700 行の `BoardPage` の
  中では `set-state-in-effect` が 1 件も出ていなかった。**「前は通っていた」は
  通っていた証拠ではない。** 出たものは 1 件ずつ判断する。導ける値を effect で
  作っているなら直し（衝突の知らせは「どの版に対する衝突か」を持つ形にした）、
  規則の対象外なら**理由をその行に書いて外す**（シーンの大きさは Excalidraw
  から読み出すしかなく、1 回ぶんの追加描画は避けられない）。**外す形に逃げて
  表示を遅らせない。**
