...
- **item ごとのリンクは、保存された Project の URL にクエリを足すだけ**
  （`projectItemLink`、ADR 0057）。土台が一覧止まり（`exact: false`）なら
  出さない。識別子が `0` や安全な整数でない行も出さない。**注記は足さない。**
- **リストごとの 1 本も残す。** Project 全体を見にいくのは別の用事。行ごとの
  リンクは同じ文言が並ぶので、アクセシブルな名前にタイトルを含める。
...
## 画面全体に出す失敗は通知で（ADR 0058）

**通知は「過ぎ去ってよい失敗」の器。残して読ませる状態は通知にしない。**
消えた瞬間に読めなくなる。次に触る人が最初に壊すのはこの線引きなので、足す前に
ここを見る。

| 出す先                                             | もの                                                                                                                     |
| -------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| 通知（`useNotify()`）                              | ログイン状態・ボード一覧・ボードを開く・シーンが読めない・注釈の状態の取得・保存（409 以外）・改名や削除などの操作の失敗 |
| 帯のまま（`.conflict`）                            | 保存の衝突（ADR 0020）。失敗ではなく状態で、上書きしていないことと次の手を読ませる                                       |
| 注釈のパネル内（`ErrorNotice`）                    | 解釈・作成の失敗と部分失敗。どの注釈で起きたかが情報の一部で、部分失敗は作られた一覧と一体（ADR 0009）                   |
| フォームの隣（`MemberPanel` / `RepositoryPicker`） | 入力に対する応答。入力欄から離れた場所に出すと、どの操作の結果か読みにくい                                               |

- **成功を通知に重ねない。** 保存の成功は「未保存」の消灯で、作成の成功は作成
  結果の一覧で示している。同じ事実が 2 箇所に出ると、どちらが状態か読めない。
- **押し直して解けるものにだけ「再試行」を付ける。** 409 と 413 は何度押しても
  同じ答えが返る。付けるなら `key` も付け、直ったら `dismissKey` で下げる。
- **通知はフローの外（`position: fixed`）。** 帯に戻すと、出た瞬間にキャンバスが
  縮んで描画位置が動く（E2E が `.canvas` の寸法で見ている）。
- **effect から出す通知には `key` を付ける。** 開発時の StrictMode では effect が
  2 回走り、畳まないと同じ失敗が 2 件並ぶ（1 本の state だった頃は上書きで
  隠れていた）。
- **Provider の外で `useNotify()` を呼ぶと投げる。** 黙って捨てる既定値は置かない。
