...
- **「未保存」判定（`web/src/excalidraw/dirty.ts`）は `content_hash` とは別物。**
  ...
  背景色（`viewBackgroundColor`）が入っているのはそのため（ADR 0044）。スクロール
...
- **取り込みの確認だけは `BoardPage` が出す**（ADR 0044）。キャンバスは外れない
...
**口は etoki のヘッダー 1 つ。** ライブラリの既定メニューにも同じことをする項目が
あるので、`BoardPage` の `UI_OPTIONS` で閉じてある。閉じているのは「開く」
（`loadScene`）と、**`.excalidraw` の書き出しである `export`**（日本語の表示が
「名前を付けて保存...」なので、下に並ぶ画像のエクスポートと紛らわしい）と、そこで
開いたファイルへの上書き（`saveToActiveFile`）。**画像のエクスポートは閉じない。**

- **書き出しはキャンバスから出す。** 保存済みシーンから出すと、未保存の描き足しが
  黙って落ちる。**保存に送る直列化と同じものを使う**（`web/src/excalidraw/transfer.ts`
  の `sceneJSON`）。保存・大きさの計測・書き出しの 3 つが同じ式を見るので、**別に
  書かない。** 書くと、ヘッダーに出ている大きさと書き出したファイルが食い違う。
- **未保存でも書き出せる。** 解釈の「保存してから」（ADR 0018）と揃えない。あちらは
  入力が 2 つあって揃える必要があるが、書き出しの入力は 1 つしかない。
- **取り込みはキャンバスに載せるだけ。** サーバーには何も送らない。確定させるのは
  人間の保存操作だけで、検証も版の照合も大きさの上限も、その保存の経路のまま効く。
- **読めるかどうかはライブラリ（`loadFromBlob`）が決める。** 自前で判定しない。
  **バイト数の上限もフロントには持たせない**（ADR 0018 / 0038 と同じ理由）。大きい
  ファイルはキャンバスに載り、ヘッダーの大きさに出て、保存で 413 になる。
- **画像は `addFiles` で入れる。** 落とすと、貼ってあった画像の要素だけが空白で
  置かれる。
- **取り込んだ絵には寄せる**（`scrollToContent`）。画面の外に置かれると、押しても
  何も起きていないように見える（図のドラフトと同じ形、ADR 0040）。
- **引いた解釈は捨てない。** 取り込みはキャンバスの編集であって保存ではない。捨てる
  のは保存したときだけ（そこは崩さない）。
- **`.excalidraw` のドロップは閉じられない。** ライブラリがシーンを置き換える。
  **`onDrop` を横取りして塞がない。** 入口が 2 つになるうえ、ドロップも未保存に
  なり、サーバーへの反映は保存を押すまで起きない。失われるのは確認だけ。
...
- **持ち出しと取り込みも E2E でしか確かめられない**（`transfer.spec.ts`、ADR 0044）。
