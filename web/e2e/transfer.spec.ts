...
test.describe("取り込み", () => {
  ...
  test("取り込んだあと元に戻すと、元のキャンバスに戻る", async ({ page }) => {
    ...
    // **取り込んだ画像は戻しても file store に残る**（addFiles は履歴に積まれない）。
    // 残ったものが保存に紛れ込まないことを、書き出しで見る。書き出しは保存と同じ
    // 直列化（`sceneJSON`）を通る。
    const [download] = await Promise.all([
      page.waitForEvent("download"),
      page.getByRole("button", { name: "書き出し" }).click(),
    ...
    const live = scene.elements.filter((el) => !el.isDeleted).map((el) => el.id);
    expect(live).not.toContain("image-imported");
    expect(Object.keys(scene.files ?? {})).not.toContain(IMAGE_FILE_ID);
  });
