...
// ぶんの終わりを待つ話で、待つ長さも次にすることも違う（ADR 0034 / 0043）。
var (
...
const (
	...
	// （ADR 0043）。窓だけは、上限を設定した人が単位を書かずに済むよう
	// 既定を持つ。
...
//
// **絞る軸は利用者**（ADR 0043）。ボード単位だと招待した相手が自分のボードを
...
//
// **呼ぶのは LLM を叩く直前**（ADR 0043）。認可と入力の検証をすべて通ったあとに
