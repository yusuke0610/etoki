...
- **解釈 1 回ぶんの実績はログに残す**（ADR 0031）。`InterpretationService.logUsage`
  ...
  - **保存しない。** 表を持つと「誰の実績か」を決める必要があり、それは実行の
    上限をどう絞るかと揃えて決める問いだった。**その軸は利用者単位に決まったが
    （ADR 0044）、据え置きは続く。** 上限が数えるのは窓の中の回数だけで窓から
    出たぶんは捨てるので、実績の表を作る理由にはならない。
...
- **LLM を叩く実行には上限がある**（ADR 0044）。`usecase.LLMLimiter` が
  **利用者ごとに**、同時実行の数と窓の中の回数を見る。ボード単位ではない。
  ボードで絞ると招待した相手が自分のボードを作れば素通りし、プロセス全体で
  絞ると他人の使いすぎで自分が止まる。空文字は「認証なしの操作者」1 人なので、
  認証なしの構成では結果としてプロセス全体の上限と一致する。
  - **解釈と図のドラフト生成で 1 つを共有する。** どちらも同じ鍵で同じモデルを
    叩くので、片方だけ絞ると抜け道が残る。`InterpretationService` と
    `DiagramService` には**同じ `*LLMLimiter` を渡す**。任意の設定ではなく
    コンストラクタの引数にしてあるのは `BoardLocks` と同じ理由。
  - **`BoardLocks` を流用しない。** あれは作成先の固定を守るためのボード単位の
    排他で、軸も目的も違う。解釈は何も書かないので直列化する対象が無い。
  - **枠を取るのは `llm.Complete` の直前。** 認可と入力の検証をすべて通ってから
    取る。手前に置くと、404 や 400 で返るリクエストが枠を食い、他人のボード ID を
    叩くだけで枠を潰せる。**当たったら LLM を 1 回も呼ばない。**
  - **見る順は同時実行 → 回数。** 逆にすると、同時実行で断ったリクエストが回数
    だけ消費する。弾いたリクエストはどちらの枠も消費しない。
  - **待たせず 429 で断り、code を 2 つに分ける**（`rate_limited` /
    `concurrency_limited`）。時間をおく話と、いま走っているぶんの終わりを待つ話で
    打ち手が違う。**上流の LLM が混んでいる場合とも別**で、そちらは
    `llm_unavailable` の 502。
  - **状態はメモリだけ。再起動で消える。** 依るのは `BoardLocks` と同じ前提
    （SQLite を複数プロセスで共有しない）で、DB に表を作らない。
  - **既定で効くのは同時実行だけ。** 回数の既定値は置かない。妥当な値は料金
    プランと使い方で決まる外の世界の値で、抱えると必ず誰かに合わない
    （ADR 0031 が金額を持たない理由と同じ）。**残り枠も返さない。** 無い上限の
    残りを常時レスポンスに載せることになる。
...
- **run の結末（`outcome`）は保存時に必ず決める（ADR 0043）。** `SaveRun` は
  ゼロ値を弾く。NULL は「記録していなかった頃の run」を表すので、書き忘れを
  通すとそこに紛れて後から見分けられない。**読むときの NULL を成功に倒さない。**
  列を足す前にも途中失敗は起きていたので、`complete` と同じに扱うと履歴が嘘を
  つく。理由（`error`）が入るのは `incomplete` のときだけ。
  **同じ食い違いは `sync_runs` の CHECK も 4 象限すべて弾く**（マイグレーション
  0011）。復元や保守 SQL は `SaveRun` を通らない。CHECK を直すには親テーブルの
  作り直しが要り、そのマイグレーションだけ外部キーを切って適用する（ADR 0049）。
  **3 状態は変わらない。** 途中で失敗しても作れたぶんは記録するので `created`
  のままで（ADR 0009）、一覧には `AnnotationStatus.lastRunOutcome` を別に出す。
- **run の記録にリクエストの `ctx` をそのまま渡さない**（ADR 0051）。接続が
  切れると `ctx` も切れ、GitHub に作ったのに記録だけが失敗する。記録は
  `WithoutCancel` と上限つきで行い、GitHub への書き込みは次の 1 件の手前で
  `ctx` を見て止める。**始めた 1 件は取り消しから切り離して最後まで待つ。**
  途中で切ると応答だけが失われ、作った ID を記録できない。止めるのを
  `port.GitHubClient` の実装に任せない。停止時は猶予が尽きる前にリクエストの
  `ctx` を切る（`etoki.go` の `cancelRequestsAfter`）。
  - **停止の猶予は `usecase.MaxCreationDrain` から導く**（ADR 0056）。固定値に
    戻すと、`itemWriteTimeout` を動かした日に猶予だけが取り残され、書き込み中の
    1 件が返る前にプロセスが終わる。**そのとき失われるのは 1 件ではなく run
    ごと**で、先に作れていた項目の記録も消える。関係は
    `TestShutdownBudgetCoversCreationDrain` が固定している。
- **手を付けた 1 件は、結果が分からなくても記録する**（ADR 0056）。
  `port.SyncItem.Confirmed` の false は「失敗した」ではなく「**GitHub に届いたか
  どうかを知らない**」。GitHub が受理したあとで応答だけを失った場合も、受理せずに
  返した場合も区別できないので、**分からない側に倒す**（ゼロ値が false なのは
  このため。ADR 0043 が結末のゼロ値を成功に倒さないのと同じ向き）。
  - **作成では item ID を空のままにする。** 埋めると、在りもしない item が更新先の
    照合（ADR 0026）を通る。更新では相手の ID が分かっているので載せる。
  - **畳み込みには入れない。** `ListItemsByAnnotation` / `ListItemsByBoard` は
    `confirmed = 1` だけを返す。未確定は `ListUnconfirmedItemsByBoard` が
    畳まずに返し、契約では `unconfirmedItems` として `items` と分ける
    （混ぜない理由は ADR 0046 と同じ）。
  - **あとで同じ item に確定の記録が付いたものは返さない。** 解けた不確かさを
    残すと打ち手の無い警告が積み上がる。**自動で GitHub に照合しにいかない**
    （中核思想 3）。
  - **3 状態は変わらない。** 確定が 1 件も無くても run があるので `created`。
    `uncreated` に戻すと、いちばん押しやすい操作（もう一度作る）が消せない重複を
    生む（ADR 0009 / 0056）。何が在るのかは件数と `unconfirmedItems` で読ませる。
  - **確定しているなら item ID がある。** `SaveRun` と `sync_items` の CHECK
    （マイグレーション 0013）の両方で弾く。空のまま確定させると、畳み込みが
    空文字で 1 つのグループを作り、別々の item が 1 件に混ざる。
...
## Gin ハンドラと cross-site リクエストの拒否

**境界 DTO・エラー写像・`GET /api/capabilities`・Origin 検証など、
`internal/httpapi/` を触るときの約束は `.claude/rules/http-handlers.md`
にある。** ここには書かない。
...
- **招待できるのは一度ログインした利用者だけ。** 未ログインの login 宛に招待を
  ...
  `etoki claim` と同じ制約。
- **login の引き当ては最後にその login でログインした 1 人だけ**（ADR 0053）。
  `UpsertUser` が同じ login（大文字小文字を無視）を持つ他の行から login を外し、
  DB も一意索引で同じ login を 2 行に持たせない。**行は消さない**（ID で
  指されている）。**招待は確かめた相手の `userId` も受け取り**、いまの持ち主と
  違えば `ErrInviteeChanged`。login だけで通すと、確認を飛ばす経路が残る。
  **GitHub には訊かない**（ADR 0017 の「招待するのに GitHub は見ない」）。
