...
- **`sync_items` の `title` と `body` は作成時点のスナップショット。**
  ...
  （ADR 0023）。詰め替えの経路（`CreationService.applyOne` →
...
- **URL は組み立てず、GitHub が返した `ProjectV2.url` を運ぶ。** Projects v2 の
  ...
  **item ごとのリンクは、この URL に `sync_items.item_database_id` をクエリで
  添えるだけ**（ADR 0057）。土台は組み立てない。識別子は作成時にしか取れない
  ので、`title` / `body` と同じく詰め替えで写し忘れないこと。ただし**畳み込み
  ではこの列だけ最新の行ではなく `MAX` を採る**（不変の属性なので、後の run が
  取り損ねても前の値を失わない）。
