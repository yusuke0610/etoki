...
//
// シーンは JSON の文字列として運ばれるので、`"` と `\` がエスケープで倍になる。
// 2 倍に包みのぶんを足して、**ユースケース層が弾くより先にここで切れないように
// する。** 先に切れると、上限ちょうどのシーンが読めずに落ちる。
const maxSceneBody = usecase.MaxSceneBytes*2 + 4<<10
