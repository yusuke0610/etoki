...
採らなかったのは、**そこに賭けても外れうる**ため。Mend が動かす hosted 版でも
`postUpgradeTasks` は使えるが、実行できるコマンドはホスト側の許可リストに限られ、
中身は公開されていない（ログの `allowedCommands` を見て確かめる形になっている）。
`nix develop` 越しの `make codegen` が回る保証は無い。self-hosted なら許可
コマンドを自分で決められるが、今度は Renovate 自体が運用するものになる。

そもそも**更新 PR の中で `make codegen` を自動実行しない**のがこのリポジトリの
方針（後述の「更新 PR は赤くなってよい」）。生成物が古いことは CI の codegen
drift が知らせ、直すのは人。そう決めた以上、Renovate に残る仕事は「更新を知る」
ことだけになり、そこまでは GitHub 標準で足りる。
...
**依存が宣言する peer 範囲の外に出ることは、それだけでは固定の理由にしない。**
`openapi-typescript` の peer は `typescript: ^5.x` だが、入っている `typescript`
は 6 系で、codegen も lint も通っている。見ているのは `make lint` /
`make test` が通るかどうかであって、宣言ではない。

先回りして範囲を狭めると、上の「落ちることが知らせ」が効かなくなる。狭めた分の
更新 PR は出なくなるので、**外部が追いついたことにも気づけない。** TypeScript を
6 系に留めたのは、上げたら eslint が規則を 1 つも読めずに起動で落ちたのを見た
あと（#118）。順序を逆にしない。
...
**`GITHUB_TOKEN` が作った PR では、workflow が承認するまで動かない**（再帰的な
実行を防ぐための GitHub の仕様。`opened` / `synchronize` / `reopened` の run は
作られるが、承認待ちで止まる）。つまり自動で作った flake.lock の PR は、**いちばん
見たい「E2E がまだ動くか」を人が承認を押すまで出さない。** 押しに行くなら、その
ついでに `nix flake update` を手で叩くのと手数が変わらない。

PAT を置けば承認は要らなくなるが、それは長命の資格情報を 1 つ増やすということ。
