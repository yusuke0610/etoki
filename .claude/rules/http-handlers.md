...
- **`/api` 全体に掛かるものは、認証の要否で分かれる前の親グループ 1 箇所に置く。**
  `noStore()`（`Cache-Control: no-store`）と `limitBody()`（本文の読み込みの
  既定の上限）がこれで、新しいグループは `r.Group("/api/...")` で生やさず親
  （`apiRoot`）にぶら下げる。認証済みのグループにだけ掛けると、**別グループと
  401 で弾かれた応答だけが漏れる。** 漏れても応答は返るので、テストを書くまで
  気づけない（#123）。
- **本文の上限は口ごとに書かない。** `limitBody()` が `defaultMaxBody` を掛け、
  広い本文を受ける口だけが `widenBody(c, ...)` で**置き換える**。
  `http.MaxBytesReader` は重ねると内側が先に切るので、重ねる形にすると広げた
  つもりの上限が効かないまま緑になる。口ごとに `http.MaxBytesReader` を書く形
  だったころ、シーン・解釈・図のドラフトに入れたあとも改名・作成先・作成・招待の
  6 つは素通しのまま残っていた（#147）。**既定で足りるかは、その口が受ける型の
  正本（ドメインの上限）で見積もる。** 作成は項目の上限から既定を超えるのに、
  既定のまま置いて、項目の上限の内側にある解釈を 413 で弾いていた（#168）。
