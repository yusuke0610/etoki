...
- **`/api` 全体に掛かるものは、認証の要否で分かれる前の親グループ 1 箇所に置く。**
  `noStore()`（`Cache-Control: no-store`）がこれで、新しいグループは
  `r.Group("/api/...")` で生やさず親（`apiRoot`）にぶら下げる。認証済みの
  グループにだけ掛けると、**別グループと 401 で弾かれた応答だけが漏れる。**
  漏れても応答は返るので、テストを書くまで気づけない（#123）。
...
- **画面の配信は `NoRoute` に置く**（ADR 0032）。ルートに当たらなかったものだけを
  ...
  **開いているボードもログイン後の戻り先もクエリで表す**ので（ADR 0056）、
