...
設計判断で迷ったらここに立ち返る。**正本は
[README](README.md#中核となる思想)。** ここに置いてあるのは同じ 3 つの文と、
実装に効く帰結。**文言を書き換えるなら README を直す。**
...
- 1 から。構造化の責務はツール側と開発者フェーズに寄せる。
- 2 から。ルールベースで推測する実装を書かない。
- 3 から。すべて開発者の手動トリガーにする。
...
| 場所                               | 中身                                                                   |
| ---------------------------------- | ---------------------------------------------------------------------- |
| `CONTRIBUTING.md`                  | ブランチ・コミット・PR 本文・レビュー対応・マージ・CI                  |
...
**一覧の正本は `make help`**（Makefile の `##` コメント）。下は使う頻度の高い
ものだけで、全部ではない。
...
**注釈の判定規則とメタデータの形が 2 箇所にある。片方だけ変えると壊れる。**

| 実装       | 場所                                                                     |
| ---------- | ------------------------------------------------------------------------ |
| Go         | `internal/domain/scene.go` の `Element.isAnnotation` と `AnnotationMeta` |
| TypeScript | `web/src/excalidraw/annotation.ts` の `isAnnotation` と `AnnotationMeta` |
...
**frame を自前生成しない線に例外は無い**（ADR 0047）。ブレストの出発点になる
...
**メタデータには粒度のほかに図の種別（`kind`）も載る**（ADR 0047）。載る先は
人が引いた frame なので、選ぶ場所は注釈パネルの「種別」（粒度の隣）。
`content_hash` の入力にも解釈のプロンプトにも入る。**種別から図の読み方を導く
実装を書かないこと。** 「シーケンス図だからこの矢印は呼び出しだ」と読み始めた
時点で、座標から構造を推測しているのと変わらない（中核思想 2）。**プロンプトに
書いても同じ。**

**ひな形の選択と種別の選択を自動で結ばないこと**（中核思想 3）。「シーケンス図の
...
**ブランチ・コミット・PR 本文・レビュー対応・マージの正本は
`CONTRIBUTING.md`。**
...
- **マージしてよいのは、最新の head に対する full review で指摘が 0 件だった PR
  だけ。** それ以外は仕上げてユーザーに渡す。条件は `CONTRIBUTING.md`。
