...
- **`BoardRepository` は所有者を引数で受け取る。** ctx から実装が勝手に読む形に
...
- **他人のボードは 404。** 403 にしない。区別すると ID の総当たりで存在を
  確かめられる。
- **`sync_runs` は所有者で絞らない。** `board_id` で引く経路しかなく、その board を
  取れるのは所有者だけ。二重に絞ると、絞り忘れたときにどちらが効いているのか
  分からなくなる。
